...
cmblens.config.argparser.add_argument('-del', '--delta',
                default='20', type=int)
...
# directory setup
postfix     = '062818_nobeam_nonoise_%d' %args.delta
...
bin_edges = np.concatenate((np.linspace(0,400,args.delta), np.linspace(400, lmax, 56)))
...
# initialize cusps
overwrite      = False
mcm_identifier = "%lsd_le%d_nb%d_lm%d_%s_%s" %(0, lmax, nbin, lmax, coords_str, postfix)
...
# bin the theory
theo_bin = {}
lbin_th, theo_bin['dltt']    = cusps_fc.bin_theory_scalarxscalar(l_th, theo['dltt'])
lbin_th, theo_bin['dlte']    = cusps_fc.bin_theory_scalarxscalar(l_th, theo['dlte'])
_, dlee_th, dleb_th, dlbb_th = cusps_fc.bin_theory_pureeb(l_th, theo['dlee'], np.zeros(len(l_th)), theo['dlbb'])

lbin_th, theo_bin['cltt']    = cusps_fc.bin_theory_scalarxscalar(l_th, theo['cltt'])
lbin_th, theo_bin['clte']    = cusps_fc.bin_theory_scalarxscalar(l_th, theo['clte'])
_, clee_th, cleb_th, clbb_th = cusps_fc.bin_theory_pureeb(l_th, theo['clee'], np.zeros(len(l_th)), theo['clbb'])
...
if cmblens.mpi.rank == 0:
    ...
    for polcomb in polcombs:
        ...
        plotter.hline(y=0, color='k')
        plotter.show_legends(fontsize=18)
        plotter.save(output_path("frac_diff%s_%s_zoom.png"%(prefix, coords_str)))
