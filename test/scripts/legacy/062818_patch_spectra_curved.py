...
def get_sim(sim_idx, beam_fwhm=beam_fwhm, noise_level=noise_level):
    ret = act_sim.getActpolCmbSim(None, coords, sim_idx, cmb_dir, doBeam=False, pixelFac= 1)
    ...
    if noise_level > 0:
        ...
        for i in range(len(tqu)): 
            ...
            nl      = cmblens.theory.get_white_noise_power(l_n, noise_level*noise_fact)
...
for idx in list(theo.keys()):
    ...
    dlidx          = 'dl%s' %idx[-2:]
    _, theo[idx], theo[dlidx] = cmblens.theory.get_total_1d_power(l_th, theo[idx], beam_fwhm, noise_level*noise_fact, True)
