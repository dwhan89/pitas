...
def generate_mcm(window_temp, window_pol, bin_edges, mcm_dir=None, lmax=None, transfer=None, save_raw=False):
    ...
    def process_mcm_process(mcm_tt, mcm_tp, mcm_pp_diag, mcm_pp_offdiag, \
        ...
        bin_mcm(mcm_tt, mbb_tt);
        bin_mcm(mcm_tp, mbb_tp)
        bin_mcm(mcm_pp_diag, mbb_pp_diag);
        bin_mcm(mcm_pp_offdiag, mbb_pp_offdiag)
    ...
    mcm_tt = np.dot(mcm_tt, mat_2dl_inv);
    mcm_tp = np.dot(mcm_tp, mat_2dl_inv)
    mcm_pp_diag = np.dot(mcm_pp_diag, mat_2dl_inv);
    mcm_pp_offdiag = np.dot(mcm_pp_offdiag, mat_2dl_inv)

    mcm_tt = np.dot(mat_2dl, mcm_tt);
    mcm_tp = np.dot(mat_2dl, mcm_tp)
    mcm_pp_diag = np.dot(mat_2dl, mcm_pp_diag);
    mcm_pp_offdiag = np.dot(mat_2dl, mcm_pp_offdiag)
