...
class PITAS(object):
    def __init__(self, mcm_identifier, window_scalar, window_pol, bin_edges, lmax=None, transfer=None, output_dir=None,
                 overwrite=False):
        ...
        self.mcm_cltt_inv = ret[0].copy();
        self.mcm_cltp_inv = ret[1].copy()
        self.mcm_clpp_inv = ret[2].copy();
        self.mcm_dltt_inv = ret[3].copy()
        self.mcm_dltp_inv = ret[4].copy();
        self.mcm_dlpp_inv = ret[5].copy()
        ...
        self.bbl_cltt = ret[0].copy();
        self.bbl_cltp = ret[1].copy()
        self.bbl_clpp = ret[2].copy();
        self.bbl_dltt = ret[3].copy()
        self.bbl_dltp = ret[4].copy();
        self.bbl_dlpp = ret[5].copy()
        del ret
...
class PITAS4FLIPPER(PITAS):
    def __init__(self, mcm_identifier, window_scalar, window_pol, bin_edges, lmax=None, transfer=None, overwrite=False):
        super(PITAS4FLIPPER, self).__init__(mcm_identifier, l2e(window_scalar), l2e(window_pol), bin_edges, lmax=lmax,
                                            transfer=transfer, overwrite=overwrite)
...
def get_bbl(mcm_identifier, window_scalar=None, window_pol=None, bin_edges=None, output_dir=None, lmax=None,
            transfer=None, overwrite=False):
    ...
    try:
        ...
        bbl_cltt = load_bbl('CLTT')
        bbl_cltp = load_bbl('CLTP')
        bbl_clpp = load_bbl('CLPP')
        bbl_dltt = load_bbl('DLTT')
        bbl_dltp = load_bbl('DLTP')
        bbl_dlpp = load_bbl('DLPP')
    except:
        if pitas.mpi.rank == 0:
...
def get_mcm_inv(mcm_identifier, window_scalar=None, window_pol=None, bin_edges=None, output_dir=None, lmax=None,
                transfer=None, overwrite=False):
    if output_dir is None: output_dir = pitas.config.get_output_dir()
    if mcm_identifier is not None: mcm_dir = os.path.join(output_dir, mcm_identifier)
    ...
    try:
        assert (not overwrite)
        mbb_cltt_inv = load_mbb_inv('CLTT_inv');
        mbb_cltp_inv = load_mbb_inv('CLTP_inv')
        mbb_clpp_inv = load_mbb_inv('CLPP_inv')

        mbb_dltt_inv = load_mbb_inv('DLTT_inv');
        mbb_dltp_inv = load_mbb_inv('DLTP_inv')
